...
class CrossCompanyUpsellSequences {
  ...
  async initializeUpsellSequence(customerId, currentCompany) {
    ...
    const opportunities = this.identifyUpsellOpportunities(
      customer,
      currentCompany,
    );
  ...
  checkCompatibility(customer, company) {
    ...
    if (
      customer.industry &&
      company.targetIndustries.includes(customer.industry)
    ) {
      score += 0.3;
    }
    ...
    if (
      ...
      company.targetCompanySize.includes(customer.companySize)
    ) {
      score += 0.2;
    }
  ...
  async startSequence(customer, opportunity) {
    ...
    const sequence = this.sequences.find(
      (s) => s.targetCompany === opportunity.company,
    );
    if (sequence) {

...
import {
  DomainTemplates,
  GalaxyEngineTemplate,
  type UniverseConfiguration,
  type UniverseConstellation,
  type UniverseEntity,
  type UniverseFlow,
} from './GalaxyEngineTemplate';
...
const UniverseCanvas: React.FC<UniverseCanvasProps> = ({
  ...
  useEffect(() => {
    const animate = () => {
      setOrbitTime(
        (prev) => prev + 0.016 * galaxyEngine.getControls().animationSpeed,
      );
  ...
  // Render universe
  const renderUniverse = useCallback(
    ...
    flows.forEach((flow) => {
      if (flow.status === 'active') {
        ...
        flow.entities.forEach((entityId, index) => {
          ...
          if (entity && index > 0) {
            const prevEntity = entities.find(
              (e) => e.id === flow.entities[index - 1],
            );
            if (prevEntity) {
              ctx.beginPath();
  ...
  const renderConstellations = (
    ...
    constellations.forEach((constellation) => {
      ...
      ctx.strokeStyle =
        constellationType?.connectionColor || config.colorScheme.secondary;
      ctx.lineWidth = 1;
      ctx.setLineDash([3, 3]);
  ...
  const renderEntity = (
    ...
    const statusColor =
      config.colorScheme.statusColors[entity.status] || entity.color;
    ...
    const avgMetric =
      Object.values(entity.metrics).reduce((sum, val) => sum + val, 0) /
  ...
  const handleCanvasClick = useCallback(
    (event: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      constellations.forEach((constellation) => {
        const distance = Math.sqrt(
          (x - constellation.centerPosition.x) ** 2 +
            (y - constellation.centerPosition.y) ** 2,
        );
    ...
    entities.forEach((entity) => {
      if (entity.orbitRadius > 0) {
        ...
        const newX =
          ...
          Math.cos(angle + entity.orbitAngle) * entity.orbitRadius;
        const newY =
          controls.centerPosition.y +
          Math.sin(angle + entity.orbitAngle) * entity.orbitRadius;
...
export const UniverseFactory: React.FC<UniverseFactoryProps> = ({
  ...
  const [galaxyEngine] = useState(
    () => new GalaxyEngineTemplate(configuration),
  );
  const [selectedEntity, setSelectedEntity] = useState<UniverseEntity | null>(
    null,
  );
    ...
    <div className={`universe-factory ${className}`}>
      <div className="universe-header">
        ...
        <div className="universe-stats">
          <span className="stat">
            🌌 {galaxyEngine.getStats().totalEntities} {configuration.domain}{' '}
            entities
          </span>
          <span className="stat">
            ...
            <div className="entity-details">
              ...
              <div className="detail-row">
                ...
                <span className="value">
                  {selectedEntity.satellites.length}
                </span>
              </div>
            </div>
...
export const BusinessIntelligenceUniverse: React.FC<
  Omit<UniverseFactoryProps, 'configuration'>
> = (props) => (
  <UniverseFactory
    configuration={DomainTemplates.BusinessIntelligence()}
    {...props}
  />
);

export const DevelopmentUniverse: React.FC<
  Omit<UniverseFactoryProps, 'configuration'>
> = (props) => (
  <UniverseFactory configuration={DomainTemplates.Development()} {...props} />
);

export const CommunicationsUniverse: React.FC<
  Omit<UniverseFactoryProps, 'configuration'>
> = (props) => (
  <UniverseFactory
    configuration={DomainTemplates.Communications()}
    {...props}
  />
);
...
export const createUniverse = (
  domain: string,
  customConfig?: Partial<UniverseConfiguration>,
): UniverseConfiguration => {
  const template = DomainTemplates[domain as keyof typeof DomainTemplates];
  if (!template) {
    throw new Error(
      `Unknown domain template: ${domain}. Available: ${Object.keys(DomainTemplates).join(', ')}`,
    );
  }

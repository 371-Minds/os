...
class EmailTrackingAnalytics {
  ...
  calculateMetricsFromEvents(events) {
    ...
    const metrics = {
      ...
      unsubscribed: events.filter((e) => e.type === 'email_unsubscribed')
        .length,
    };
  ...
  async getCampaignPerformanceReport(filters) {
    ...
    for (const campaignId of campaigns) {
      if (campaignId) {
        performance[campaignId] = await this.getCampaignMetrics(
          campaignId,
          filters.timeRange,
        );
      }
  ...
  async getAudienceInsightsReport(filters) {
    ...
    events.forEach((event) => {
      ...
      if (event.metadata && event.metadata.userAgent) {
        ...
        insights.deviceTypes[deviceType] =
          (insights.deviceTypes[deviceType] || 0) + 1;
      }

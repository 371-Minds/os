...
export const ReadersConstellation: React.FC<ReadersConstellationProps> = ({
  ...
  const [books, setBooks] = useState<Book[]>([
    {
      ...
      connections: ['2', '3'],
      insights: ['Code is a craft, not just engineering', 'DRY principle transforms development'],
    ...
    },
    {
      ...
      connections: ['1', '4'],
      insights: ['Dependencies should point inward', 'Architecture screams the intent'],
    ...
    },
    {
      ...
      connections: ['4', '8'],
      insights: ['System 1 is fast, System 2 is deliberate', 'Biases shape all decisions'],
    ...
    },
    {
      ...
      connections: ['6'],
      insights: ['Stories unite humanity', 'Cognitive revolution changed everything'],
  ...
  const drawConstellations = (ctx: CanvasRenderingContext2D) => {
    constellations.forEach(constellation => {
      const constellationBooks = books.filter(book => constellation.books.includes(book.id));
  ...
  const drawBooks = (ctx: CanvasRenderingContext2D) => {
    books.forEach(book => {
      ...
      if (progress === 1 && book.rating) {
        drawRatingStars(ctx, book.position.x, book.position.y + size + 20, book.rating);
      }
  ...
  const drawBookLabel = (ctx: CanvasRenderingContext2D, book: Book, isSelected: boolean) => {
    const padding = 12;
    const lineHeight = 16;
    const maxWidth = 200;
    ...
    lines.forEach((line, index) => {
      ...
      ctx.fillText(line, labelX + padding, labelY + padding + (index + 1) * lineHeight);
    });
  };

  // Draw UI overlays and controls
  const drawUIOverlays = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) => {
    ...
    ctx.fillStyle = '#cbd5e1';
    ctx.font = '11px Inter, sans-serif';
    ctx.fillText(`Zoom: ${(universeState.zoomLevel * 100).toFixed(0)}%`, canvas.width - 110, 28);
  };
    ...
    if (isDragging) {
      ...
      const hoveredBook = books.find(book => {
        const distance = Math.sqrt(
          Math.pow(x - book.position.x, 2) + Math.pow(y - book.position.y, 2)
        );
  ...
  const handleMouseDown = (event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (clickedBook) {
      setUniverseState(prev => ({
        ...prev,
        selectedBook: prev.selectedBook === clickedBook.id ? null : clickedBook.id
      }));
  ...
  const handleMarkProgress = (bookId: string, pages: number) => {
    setBooks(prev => prev.map(book => 
      book.id === bookId 
        ? { ...book, readPages: Math.min(book.pages, book.readPages + pages), lastRead: new Date() }
        : book
    ));
  ...
  const selectedBook = books.find(book => book.id === universeState.selectedBook);
    ...
    <div className="readers-constellation">
      <div className="universe-header">
        ...
        <div className="universe-stats">
          ...
          <span>🎯 {books.filter(b => b.readPages === b.pages).length} Completed</span>
        </div>
      </div>
          ...
          <div className="book-details-panel">
            <div className="book-details-header">
              ...
              <button 
                ...
                onClick={() => setUniverseState(prev => ({ ...prev, selectedBook: null }))}
              >
                ×
              </button>
            </div>
            ...
            <div className="book-details-content">
              ...
              <div className="book-progress">
                ...
                <span className="progress-text">
                  {selectedBook.readPages} / {selectedBook.pages} pages ({Math.round((selectedBook.readPages / selectedBook.pages) * 100)}%)
                </span>
              </div>
              ...
              <div className="book-actions">
                ...
                <button 
                  className="action-btn"
                  onClick={() => handleAddInsight(selectedBook.id, 'New insight captured from reading session')}
                >
                  💡 Add Insight
                </button>
              </div>
      ...
      <div className="universe-controls">
        <div className="control-group">
          ...
          <input 
            ...
            onChange={(e) => setUniverseState(prev => ({ 
              ...prev, 
        ...
        <div className="control-actions">
          <button className="control-btn" onClick={() => console.log('Add Book to Universe')}>
            📚 Add Book
          </button>
          <button className="control-btn" onClick={() => console.log('Create New Constellation')}>
            ⭐ New Constellation
          </button>
          <button className="control-btn" onClick={() => console.log('Export Reading Universe')}>
            🌌 Export Universe
          </button>
        </div>
      </div>

...
export const QuarterlyReview: React.FC<QuarterlyReviewProps> = ({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'exceeded': return '#27ae60';
      case 'met': return '#3498db';
  ...
  return (
    <div style={{ fontFamily: 'Arial, sans-serif', maxWidth: '600px', margin: '0 auto' }}>
      ...
      <p>
        We're pleased to share your quarterly performance review for {quarter} {year}. 
        This comprehensive report highlights our progress, achievements, and strategic 
        focus areas for the upcoming quarter.
      </p>
      
      <div style={{ backgroundColor: '#f8f9fa', padding: '20px', borderRadius: '5px', margin: '20px 0' }}>
        <h3 style={{ color: '#2c3e50', marginTop: '0' }}>Performance Metrics</h3>
        {metrics.map((metric, index) => (
          <div key={index} style={{ 
            ...
            alignItems: 'center',
            padding: '10px 0',
            borderBottom: index < metrics.length - 1 ? '1px solid #ecf0f1' : 'none'
          }}>
            <div style={{ flex: 1 }}>
      ...
      <div style={{ backgroundColor: '#fdf2e8', padding: '20px', borderRadius: '5px', margin: '20px 0' }}>
        <h3 style={{ color: '#2c3e50', marginTop: '0' }}>Challenges & Learnings</h3>
        <ul>
      ...
      <div style={{ backgroundColor: '#e8f4fd', padding: '20px', borderRadius: '5px', margin: '20px 0' }}>
        <h3 style={{ color: '#2c3e50', marginTop: '0' }}>Next Quarter Focus</h3>
        <ol>
          {nextQuarterFocus.map((focus, index) => (
            <li key={index} style={{ marginBottom: '8px' }}>{focus}</li>
          ))}
        </ol>
      </div>
      ...
      <p>
        Based on this quarter's performance and market insights, we're well-positioned 
        to accelerate growth in the coming months. We'll continue to monitor key metrics 
        closely and adjust our strategies to ensure optimal results.
      </p>
      
      <p>
        I'll be scheduling our quarterly review meeting within the next week to discuss 
        these results in detail and finalize our strategy for the next quarter.
      </p>

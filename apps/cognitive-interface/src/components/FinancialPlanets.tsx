...
export const FinancialPlanets: React.FC<FinancialPlanetsProps> = ({
  ...
  const createPlanets = useCallback((data: any[]): FinancialPlanet[] => {
    return data.map((metric, index) => {
      const planetSize = Math.max(8, Math.min(25, (metric.value / 1000000) * 2));
      ...
      return {
        ...
        growth: ((metric.value - (metric.previousValue || metric.value)) / metric.value) * 100,
  ...
  const updatePositions = useCallback(() => {
    setPlanets(prev => prev.map(planet => {
      ...
      const newX = 400 + Math.cos(angle + planet.orbitAngle) * (planet.orbitRadius + wobble);
      const newY = 300 + Math.sin(angle + planet.orbitAngle) * (planet.orbitRadius + wobble);
  ...
  const renderPlanets = useCallback((ctx: CanvasRenderingContext2D) => {
    planets.forEach(planet => {
      const { x, y } = planet.position;
      
      // Planet atmosphere
      const atmosphereGradient = ctx.createRadialGradient(x, y, planet.size, x, y, planet.size + 8);
  ...
  const handleMouseMove = useCallback((event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const hoveredPlanet = planets.find(planet => {
      const distance = Math.sqrt(Math.pow(x - planet.position.x, 2) + Math.pow(y - planet.position.y, 2));
  ...
  const handleClick = useCallback((event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (clickedPlanet) {
      setSelectedPlanet(prev => prev === clickedPlanet.id ? null : clickedPlanet.id);

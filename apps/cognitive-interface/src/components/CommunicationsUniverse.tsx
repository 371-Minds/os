...
export const CommunicationsUniverse: React.FC<CommunicationsUniverseProps> = ({
  ...
  const getSatelliteType = (index: number): EmailSatellite['type'] => {
    const types: EmailSatellite['type'][] = [
      'metric',
      'metric',
      'recipient',
      'link',
    ...
    setUniverseEmails((prev) =>
      prev.map((email) => {
        ...
        const newX =
          ...
          Math.cos(angle + email.orbitAngle) * email.orbitRadius;
        const newY =
          controls.centerPosition.y +
          Math.sin(angle + email.orbitAngle) * email.orbitRadius;
    ...
    flows.forEach((flow) => {
      if (flow.status === 'active') {
        ...
        flow.emails.forEach((emailId, index) => {
          ...
          if (email && index > 0) {
            const prevEmail = universeEmails.find(
              (e) => e.id === flow.emails[index - 1],
            );
            if (prevEmail) {
              ctx.beginPath();
  ...
  const renderScheduledIndicator = (
    ctx: CanvasRenderingContext2D,
    x: number,
    y: number,
    size: number,
  ...
  const renderEngagementMetrics = (ctx: CanvasRenderingContext2D) => {
    ...
    const avgEngagement =
      universeEmails.reduce((sum, email) => sum + email.engagementScore, 0) /
        totalEmails || 0;
    const totalRecipients = universeEmails.reduce(
    ...
    (event: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      const clickedEmail = universeEmails.find((email) => {
        const distance = Math.sqrt(
          (x - email.position.x) ** 2 + (y - email.position.y) ** 2,
        );
      ...
      if (clickedEmail) {
        setControls((prev) => ({
          ...prev,
          selectedEmail:
            prev.selectedEmail === clickedEmail.id ? null : clickedEmail.id,
        }));

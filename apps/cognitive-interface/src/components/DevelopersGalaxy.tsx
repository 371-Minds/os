...
export const DevelopersGalaxy: React.FC<DevelopersGalaxyProps> = ({
  ...
  const generateModules = (projectId: string): CodeModule[] => {
    ...
    for (let i = 0; i < moduleCount; i++) {
      ...
      modules.push({
        ...
        type: ['component', 'service', 'util', 'test'][Math.floor(Math.random() * 4)] as CodeModule['type'],
        position: { x: 0, y: 0 },
        orbitRadius,
        orbitSpeed: 0.02 + (Math.random() * 0.02),
        size: 3 + Math.random() * 3,
        quality: 70 + Math.random() * 30,
        color: getModuleColor(['component', 'service', 'util', 'test'][Math.floor(Math.random() * 4)])
      });
    }
  ...
  useEffect(() => {
    setGalaxyProjects(prev => prev.map(project => {
  ...
  // Render galaxy
  const renderGalaxy = useCallback((ctx: CanvasRenderingContext2D) => {
    ...
    buildPipelines.forEach(pipeline => {
      if (pipeline.status === 'running') {
        ...
        pipeline.projects.forEach((projectId, index) => {
          ...
          if (project && index > 0) {
            const prevProject = galaxyProjects.find(p => p.id === pipeline.projects[index - 1]);
            if (prevProject) {
              ctx.beginPath();
  ...
  const renderProject = (ctx: CanvasRenderingContext2D, project: CodeProject) => {
    ...
    if (controls.showMetrics) {
      ctx.strokeStyle = project.health > 90 ? '#10b981' : project.health > 70 ? '#f59e0b' : '#ef4444';
      ctx.lineWidth = 2;
      ctx.shadowBlur = 5;
      ctx.beginPath();
    ...
    if (controls.viewMode === 'system' || project.isSelected) {
      project.modules.forEach(module => {
        ...
        ctx.shadowBlur = 3;
        ctx.beginPath();
        ctx.arc(module.position.x, module.position.y, module.size, 0, Math.PI * 2);
        ctx.fill();
      });
    }
  ...
  const renderBuildingEffect = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number) => {
  ...
  const renderFailureEffect = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number) => {
  ...
  const handleCanvasClick = useCallback((event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (clickedProject) {
      ...
      setGalaxyProjects(prev => prev.map(p => ({
        ...p,
    ...
    if (controls.viewMode === 'system') {
      for (const project of galaxyProjects) {
        const clickedModule = project.modules.find(module => {
          const distance = Math.sqrt(
            Math.pow(x - module.position.x, 2) + Math.pow(y - module.position.y, 2)
          );
  ...
  }, [galaxyProjects, controls.viewMode, onProjectSelect, onModuleSelect, canvasRef]);

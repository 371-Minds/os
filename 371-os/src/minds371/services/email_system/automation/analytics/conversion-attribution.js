...
class ConversionAttributionAnalytics {
  ...
  async performAttribution(conversion) {
    ...
    if (relevantTouchpoints.length === 0) {
      console.log(
        'No relevant touchpoints found for conversion:',
        conversion.id,
      );
      return;
    }
  ...
  async calculateAttribution(conversion, touchpoints) {
    ...
    switch (this.attributionModel) {
      ...
      case 'linear':
        ...
        break;
      case 'time_decay':
        attribution = this.timeDecayAttribution(
          touchpoints,
          conversion.timestamp,
        );
        break;
    ...
    Object.keys(attribution).forEach((key) => {
      attribution[key].attributedValue =
        attribution[key].weight * conversion.conversionValue;
    });
  ...
  timeDecayAttribution(touchpoints, conversionTimestamp) {
    ...
    const weights = touchpoints.map((tp) => {
      const daysDiff =
        (conversionTimestamp - tp.timestamp) / (1000 * 60 * 60 * 24);
  ...
  positionBasedAttribution(touchpoints) {
    ...
    attribution[lastTouchpoint.campaignId].touchpointIds.push(
      lastTouchpoint.id,
    );
  ...
  async getAttributionReport(filters = {}) {
    ...
    if (filters.companyId) {
      conversions = conversions.filter(
        (c) => c.companyId === filters.companyId,
      );
    }

    if (filters.conversionType) {
      conversions = conversions.filter(
        (c) => c.conversionType === filters.conversionType,
      );
    }
    ...
    conversions.forEach((conversion) => {
      ...
      if (conversion.attribution) {
        Object.values(conversion.attribution).forEach((attr) => {
          ...
          report.campaignAttribution[attr.campaignId].attributedValue +=
            attr.attributedValue;
          ...
          report.companyAttribution[attr.companyId].attributedValue +=
            attr.attributedValue;
  ...
  async getCustomerJourney(contactEmail, timeRange = null) {
    ...
    let touchpoints = this.touchpoints.filter(
      (tp) => tp.contactEmail === contactEmail,
    );
    let conversions = this.conversions.filter(

...
export const getDependencyGraphAction: Action = {
  ...
  description:
    'Retrieves the complete dependency graph of the Nx workspace, showing how all projects are interconnected',
        ...
        callback({
          ...
          content: {
            ...
            analysis: {
              ...
              totalDependencies: Object.keys(graph.dependencies).reduce(
                (acc, key) => acc + graph.dependencies[key].length,
                0,
              ),
              projectsByType: Object.values(graph.nodes).reduce(
                (acc, node) => {
...
export const findAffectedProjectsAction: Action = {
  ...
  description:
    'Identifies all projects affected by recent changes, enabling targeted builds and tests',
  validate: async (runtime: IAgentRuntime, message: Memory) => {
    return true;
  },
  handler: async (
    ...
    try {
      ...
      const affected: NxAffectedProjects =
        await provider.findAffectedProjects(base);
...
export const runTestsForAffectedAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      if (callback) {
        const passedProjects = Object.entries(results.projects).filter(
          ([_, result]) => result.success,
        );
        const failedProjects = Object.entries(results.projects).filter(
          ([_, result]) => !result.success,
        );
...
export const buildProjectAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      if (callback) {
        const successfulBuilds = Object.entries(results.projects).filter(
          ([_, result]) => result.success,
        );
        const failedBuilds = Object.entries(results.projects).filter(
          ([_, result]) => !result.success,
        );
...
export const generateScaffoldAction: Action = {
  ...
  description:
    'Generates new projects, applications, libraries, or components using Nx generators',
  validate: async (runtime: IAgentRuntime, message: Memory) => {
...
export const analyzeWorkspaceAction: Action = {
  ...
  description:
    'Performs comprehensive analysis of the workspace architecture and provides optimization recommendations',
  validate: async (runtime: IAgentRuntime, message: Memory) => {
    return true;
  },
  handler: async (
            ...
            insights: {
              healthScore:
                analysis.circularDependencies.length === 0
                  ? 'Excellent'
                  : analysis.circularDependencies.length < 3
                    ? 'Good'
                    : 'Needs Attention',
              complexityLevel:
                analysis.dependencyComplexity < 10
                  ? 'Low'
                  : analysis.dependencyComplexity < 25
                    ? 'Medium'
                    : 'High',
            },
          },

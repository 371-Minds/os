...
export class NxWorkspaceProvider {
  ...
  async getDependencyGraph(focus?: string): Promise<NxDependencyGraph> {
    try {
      ...
      throw new Error(
        `Failed to generate dependency graph: ${(error as Error).message}`,
      );
    }
  }
  ...
  async findAffectedProjects(base = 'main'): Promise<NxAffectedProjects> {
    try {
      ...
      const projectsOutput = execSync(
        `npx nx affected:apps --base=${base} --plain`,
        {
          cwd: this.workspaceRoot,
          encoding: 'utf-8',
        },
      ...
      const libsOutput = execSync(
        `npx nx affected:libs --base=${base} --plain`,
        {
          cwd: this.workspaceRoot,
          encoding: 'utf-8',
        },
      ...
      // Parse project names
      const apps = projectsOutput
        .trim()
        .split('\n')
        .filter((line) => line.trim());
      const libs = libsOutput
        .trim()
        .split('\n')
        .filter((line) => line.trim());
      ...
      const tasksOutput = execSync(
        `npx nx affected --target=build --base=${base} --dry-run`,
        {
          cwd: this.workspaceRoot,
          encoding: 'utf-8',
        },
      ...
      throw new Error(
        `Failed to find affected projects: ${(error as Error).message}`,
      );
    }
  }
  ...
  async generateScaffold(options: NxGenerateOptions): Promise<boolean> {
    try {
      ...
      Object.entries(options).forEach(([key, value]) => {
        if (
          !['type', 'name', 'directory', 'tags'].includes(key) &&
          value !== undefined
        ) {
          command += ` --${key}=${value}`;
        }
  ...
  async analyzeWorkspace(): Promise<WorkspaceAnalysis> {
    try {
      ...
      const dependencyComplexity = Object.keys(graph.dependencies).reduce(
        (acc, key) => {
      ...
      const orphanedProjects = Object.keys(graph.nodes).filter((project) => {
        ...
        const hasOutgoing =
          graph.dependencies[project] && graph.dependencies[project].length > 0;
      ...
      if (orphanedProjects.length > 0) {
        recommendations.push(
          `Consider consolidating or removing ${orphanedProjects.length} orphaned projects: ${orphanedProjects.join(', ')}`,
        );
      }

      if (dependencyComplexity > 50) {
        recommendations.push(
          'High dependency complexity detected. Consider creating shared libraries to reduce coupling.',
        );
      }
      ...
      if (appCount > 0 && libCount === 0) {
        recommendations.push(
          'Consider extracting shared functionality into libraries for better reusability.',
        );
      }
      ...
      throw new Error(
        `Failed to analyze workspace: ${(error as Error).message}`,
      );
    }
  }
}

...
import { CognitiveState, CognitiveContext, UIMode } from './types';
...
export function isValidCognitiveState(state: string): state is CognitiveState {
  const validStates: CognitiveState[] = [
    ...
    'analytical', 'collaborative', 'learning'
  ];
...
export function calculateCognitiveLoad(context: CognitiveContext): number {
  ...
  switch (context.focusLevel) {
    case 'low': load += 0.3; break;
    case 'medium': load += 0.1; break;
    case 'high': load -= 0.1; break;
  }
  
  // Recent actions complexity
  const complexActions = context.recentActions.filter(action => 
    action.includes('DEBUG') || action.includes('ANALYZE') || action.includes('COMPLEX')
  );
...
export function generateStateRecommendations(
  ...
  const recommendations: Array<{ state: CognitiveState; reason: string; confidence: number }> = [];
  ...
  if (cognitiveLoad > 0.7) {
    if (currentState === 'technical' || currentState === 'analytical') {
      recommendations.push({
        state: 'creative',
        reason: 'High cognitive load detected - creative work may provide mental refreshment',
        confidence: 0.8
      });
    }
  }
...
export function formatCognitiveContext(context: CognitiveContext): string {
  ...
  if (context.recentActions.length > 0) {
    parts.push(`Last: ${context.recentActions[context.recentActions.length - 1]}`);
  }
...
export function calculateTransitionCompatibility(
  ...
  const compatibility: Record<CognitiveState, Record<CognitiveState, number>> = {
    executive: {
      executive: 1.0, technical: 0.6, creative: 0.7, 
      analytical: 0.8, collaborative: 0.9, learning: 0.5
    },
    technical: {
      executive: 0.5, technical: 1.0, creative: 0.4,
      analytical: 0.8, collaborative: 0.3, learning: 0.6
    },
    creative: {
      executive: 0.6, technical: 0.4, creative: 1.0,
      analytical: 0.5, collaborative: 0.7, learning: 0.8
    },
    analytical: {
      executive: 0.7, technical: 0.8, creative: 0.5,
      analytical: 1.0, collaborative: 0.6, learning: 0.9
    },
    collaborative: {
      executive: 0.8, technical: 0.3, creative: 0.7,
      analytical: 0.6, collaborative: 1.0, learning: 0.5
    },
    learning: {
      executive: 0.4, technical: 0.6, creative: 0.8,
      analytical: 0.9, collaborative: 0.5, learning: 1.0
    }
  };
...
export function generateUniverseConfig(
  ...
  const config = baseConfigs[userType as keyof typeof baseConfigs] || baseConfigs.executive;
  ...
  if (preferences.minimalMode) config.interactions = config.interactions.slice(0, 2);
...
export function validateUIMode(mode: UIMode): boolean {
  const required = ['name', 'description', 'cognitiveState', 'primaryActions', 'layoutConfig'];

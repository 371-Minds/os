...
function evaluateResponseQuality(response: string): number {
  ...
  const trendWords = ['growth', 'increase', 'decrease', 'trend', 'rising', 'falling', 'stable'];
  ...
  const timeWords = ['quarter', 'month', 'year', 'week', 'daily', 'annual', 'ytd', 'qtd'];
  ...
  const segmentWords = ['enterprise', 'smb', 'department', 'division', 'segment', 'channel'];
  ...
  const futureWords = ['forecast', 'predict', 'expect', 'project', 'target', 'goal'];
...
function evaluateBusinessRelevance(response: string, message: Memory): number {
  ...
  const businessTerms = [
    'revenue', 'profit', 'margin', 'roi', 'kpi', 'metrics', 'performance',
    'efficiency', 'productivity', 'budget', 'cost', 'investment', 'return',
    'market', 'customer', 'growth', 'strategy', 'competitive', 'analysis'
  ];
  ...
  const contextTerms = [
    ...
    'saas', 'enterprise', 'smb', 'subscription', 'recurring'
  ];
...
function evaluateActionability(response: string): number {
  ...
  const actionWords = [
    'recommend', 'suggest', 'should', 'need to', 'must', 'consider',
    'implement', 'focus on', 'prioritize', 'action', 'next step'
  ];
  ...
  const outcomeWords = ['target', 'goal', 'metric', 'measure', 'track', 'monitor'];
...
function extractBusinessInsights(response: string, runtime: IAgentRuntime): AgentInsight[] {
...
function generateFollowupSuggestions(response: string, qualityScore: number): string[] {
  const suggestions: string[] = [];
  
  if (qualityScore < 70) {
    suggestions.push('Request more specific metrics and data points');
    suggestions.push('Ask for time-based comparisons (month-over-month, year-over-year)');
  }
  ...
  if (!response.toLowerCase().includes('forecast') && !response.toLowerCase().includes('predict')) {
    suggestions.push('Request predictive analysis and future projections');
  }
  
  if (!response.toLowerCase().includes('department') && !response.toLowerCase().includes('segment')) {
